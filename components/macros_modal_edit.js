...
export default function MacrosModal(props){
  ...
  const findPedalObjectIndex = (pedal) => pedals.indexOf(findPedalObject(pedal))
  ...
  const saveMacro = ()=>{
    ...
    macroDispatch({type: 'update-macro', data: updatedData, macro_id: macroToEdit.macro_id})
    setMacrosModalOpen()
    setMacroToEdit(null)
  }
  ...
  return (
      ...
      <div className="menu-select">
        <CloseButton
          setHeaderOpen={setMacrosModalOpen}
          headerOpen={true} />
          ...
          <div className="sysex-menu fade-in">
            <div className="presets-container">
              ...
              {pedals.map((pedal, index)=>{
                return <a key={index} href="#" onClick={()=>{showOrHidePedal(pedal)}}>{pedal.name}</a>
              })}
              {pedals.map((pedal, index)=>{
                if(pedal.showing){
                  ...
                  let setSelectedPreset = (option)=>{
                    ...
                    let optionVals = findPresets(pedal).filter(x=> x.label == option)[0]
                    copiedPedalsConfig[index].selectedPreset = optionVals
                    setPedalState(copiedPedalsConfig)
                  }
                  return <div key={index} className="options-block">
                    <CustomSelect options={options} defaultOption={pedal.selectedPreset.label} onChange={setSelectedPreset}/>
                    </div>
                }
              })}
              <div>
