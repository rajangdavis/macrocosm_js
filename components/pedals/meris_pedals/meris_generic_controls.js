...
export default function MerisGenericControls(props){
  ...
  let fetchComponentByType = (localControl, key, i)=>{
    let localControlWithValue = {...localControl, value: props.pedalData[key]};
    if(localControl.type == 'knob'){
      return (
        <GenericKnob
        pedalName={pedalName}
        controlChange={props.controlChange}
        className={localControl.className}
        key={i}
        pedalFunction={key}
        mappedTo={localControlWithValue}
        deviceOutput={props.deviceOutput}
      ...
      return (
        ...
        changeAltState={props.changeAltState}/>
      )
    }
  }
